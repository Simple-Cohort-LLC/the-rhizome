...
import { fetchUserCasts } from "../../api/Backend";
import { useNavigation } from "@react-navigation/native";
import { sortByScore } from "../../utils/sort";
import { HEADER_HEIGHT } from "../organisms/Header";

const Profile = () => {
  const { fid, isDarkMode, isAuthenticated } = useApp();
  const [casts, setCasts] = useState([]);
  const [isDemo, setIsDemo] = useState(false);
  const [loading, setLoading] = useState(false);

  const fetchCasts = async (fid) => {
    const data = await fetchUserCasts(fid);

    const sortedCasts = sortByScore(data);
  ...
  useEffect(() => {
    if (loading || casts.length) return;
    setLoading(true);

    fetchCasts(fid);
  }, [fid]);

  const RenderCast = React.memo(({ item }) => {
  ...
  const renderItem = useCallback(({ item }) => <RenderCast item={item} />, []);
  ...
  if (!isAuthenticated && !isDemo) {
    return (
      <View style={styles.loader}>
        <Text>Sign in with your Farcaster account to view your profile.</Text>

        <TouchableOpacity
          onPress={() => {
            fetchCasts(6741); // sean07.eth
            setIsDemo(true);
          }}
          style={styles.demoBtn}
        >
          <Text style={styles.demoBtnText}>View Demo</Text>
        </TouchableOpacity>
      </View>
    );
  }
